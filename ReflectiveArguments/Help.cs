...
namespace ReflectiveArguments
{
    class Help
    {
        ...
        public List<string> GetHelp()
        {
            ...
            var options = command.ExplicitArguments.Select(x => (Left: $"--{x.KebabName}={x.DataType.Name}", Right: x.Description));
            var arguments = command.ImplicitArguments.Select(x => (Left: $"{x.SnakeName} ({x.DataType.Name})", Right: x.Description));
